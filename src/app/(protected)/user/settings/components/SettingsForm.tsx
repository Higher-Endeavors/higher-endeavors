...
const SettingsForm = () => {
  ...
  // Server Action state
  const [formState, formAction, isMutating] = useActionState<UpdateUserSettingsState, FormData>(async (prevState, formData) => {
    const result = await updateUserSettings(prevState, formData);
    if (result?.success) {
