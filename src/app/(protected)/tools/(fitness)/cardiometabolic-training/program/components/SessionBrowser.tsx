...
export default function ProgramBrowser({ 
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      clientLogger.error('Error deleting CME session', error);
  ...
  const handleDuplicateConfirm = async () => {
    if (!sessionToDuplicate || !newSessionName.trim()) return;
    ...
    try {
      ...
      clientLogger.error('Error duplicating CME session', error);
      setError(error instanceof Error ? error.message : 'Failed to duplicate session');
    } finally {
  ...
  return (
    <div className="bg-gray-100 dark:bg-[#e0e0e0] rounded-lg shadow p-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-slate-900">Saved Sessions</h2>
        <div className="flex items-center space-x-2">
          {isSessionLoaded && newSessionHandler && (
            <button
      ...
      {isOpen && (
        <div className="mt-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="flex items-center space-x-4">
                ...
                <label className="flex items-center space-x-2">
                  <input
                    type="checkbox"
                    checked={filters.hideOwnSessions}
                    onChange={(e) => setFilters({ ...filters, hideOwnSessions: e.target.checked })}
          ...
          <div className="space-y-4 mb-6">
            <input
              type="text"
              placeholder="Search sessions, templates, or activities..."
          ...
          {/* Items List */}
          {isLoading.sessions && !hasAttemptedLoad ? (
            ...
            <div className="flex flex-col items-center justify-center py-8 text-gray-500 dark:text-slate-600">
              ...
              <p className="text-sm text-center">
                {filters.showTemplates && filters.hideOwnSessions 
                  ? "No templates match your search criteria"
                  : "Create your first CardioMetabolic Endurance Session to get started"
                }
              </p>
            </div>
          ) : (
        ...
        <Modal.Body>
          <div className="space-y-4">
            ...
            <div className="flex justify-end space-x-3">
              ...
              >
                {isLoading.delete ? 'Deleting...' : 'Delete Session'}
              </button>
            </div>
          </div>
        </Modal.Body>
      ...
      >
        ...
        <Modal.Body>
          <div className="space-y-4">
            <p className="text-gray-700 dark:text-gray-100">
              Enter a name for the duplicated session:
            </p>
            <input
              type="text"
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button
                ...
                disabled={isLoading.duplicate || !newSessionName.trim()}
                className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50"
              >
                {isLoading.duplicate ? 'Duplicating...' : 'Duplicate Session'}
              </button>
            </div>
          </div>
        </Modal.Body>
