...
export default function AddExerciseModal({ isOpen, onClose, onAdd, exercises, userId }: AddExerciseModalProps) {
  ...
  const exerciseOptions: ExerciseOption[] = sortedExercises
    .filter(exercise => exercise.exerciseLibraryId !== undefined)
    .map(exercise => ({
  ...
  const onSubmit = (data: AddExerciseFormValues) => {
    ...
    const newExercise: ProgramExercisesPlanned = {
      programExercisesPlannedId: 0, // Will be set by the server
      resistanceProgramId: 0, // Will be set by the program context
