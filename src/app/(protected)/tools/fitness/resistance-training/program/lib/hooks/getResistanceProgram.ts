...
export async function getResistanceProgram(programId: number, userId: number): Promise<{
  ...
  const program: ResistanceProgram = {
    ...
    resistPhaseId: data.resist_phase_id ?? null,
    resistPhaseName: data.resist_phase_name ?? undefined,
    resistPeriodizationId: data.resist_periodization_id ?? null,
    resistPeriodizationName: data.resist_periodization_name ?? undefined,
