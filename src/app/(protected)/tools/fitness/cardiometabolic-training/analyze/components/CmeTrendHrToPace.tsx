...
export default function CMETrendHrToPace({ labels, values, unit, fixedHr, names = [], durationsSec = [], distancesMeters = [], distanceUnit = 'miles' as 'miles' | 'km' | 'm' }: { labels: string[]; values: number[]; unit: 'min/mi' | 'min/km'; fixedHr: number; names?: string[]; durationsSec?: number[]; distancesMeters?: number[]; distanceUnit?: 'miles' | 'km' | 'm' }) {
  const data = {
    labels,
    datasets: [
      {
        label: `Avg Pace @ ${fixedHr} bpm (${unit})`,
        data: values,
        borderColor: '#2563eb',
  ...
  const options: any = {
      ...
      tooltip: {
        callbacks: {
          ...
          label: (ctx: any) => {
            ...
            const pace = `${formatMinutes(ctx.parsed.y)} ${unit === 'min/mi' ? '/mi' : '/km'}`;
            ...
            const parts = [pace, name, dur, dist].filter(Boolean);
    ...
    scales: {
      ...
      y: {
        title: { display: true, text: unit === 'min/mi' ? 'Pace (min/mi)' : 'Pace (min/km)' },
        beginAtZero: false,
        ticks: { callback: (v: number) => formatMinutes(Number(v)) },
